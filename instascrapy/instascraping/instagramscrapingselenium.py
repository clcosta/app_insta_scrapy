...
from .exceptions import PrivateProfileError
...
class InstagramScrapingSelenium:
    ...
    def esperar_post_carregar(self):
        ...
        element = wait.until(
            EC.element_to_be_clickable(
                (By.XPATH, '//div[@class="v1Nh3 kIKUG  _bz0w"]/a')
            )
        )
    ...
    def pegar_n_posts(self):
        ...
        element = wait.until(
            EC.presence_of_element_located(
                (By.XPATH, '//span[@class="g47SY "]')
            )
        )
        n_posts = self.driver.find_elements_by_xpath(
            '//span[@class="g47SY "]'
        )[0].text
        if "." in n_posts:
    ...
    def __tempo_de_espera_seguranca(self, time_s, multiplicador: int = 6):
        ...
        for t in track(
            range(execucao),
            f"Tempo até a extração do proximo perfil: {time_s*multiplicador} segundos",
        ):
            time.sleep(1)

    def scraping_posts_infos(
        self, lista_de_perfils: list, sleep_time: int, between: int
    ):
        ...
        for j, user in enumerate(lista_de_perfils):
            ...
            privado, usuario = self.__check_is_private(user)
            if privado:
                raise PrivateProfileError(user=usuario)
            ...
            with self.console.status(
                f"[green]Realizando o scraping dos detalhes posts deste perfil!"
            ) as scraping:
                for i, post in enumerate(posts):
                    if i % between == 0 and i != 0:
                        self.console.log(
                            f"Detalhes dos posts: {i}/{len(posts)}"
                        )
                    ...
                    (
                        ...
                        views_count,
                        date,
                    ) = self.scraping_posts_details(post=post, user=user)
                    if None in (likes, comments, is_video, views_count, date):
                        return super_dict
            ...
            self.console.log(
                f"Extração do perfil {user} realizada com [green]sucesso![/]"
            )

        self.console.log(
            "Scraping dos detalhes dos posts [green]Finalizado[/]"
        )
    ...
    def scraping_posts_details(
        ...
        if not os.path.exists(instascraping_cache):
            os.mkdir(instascraping_cache)  ### Create Cache dir
    ...
    def get_posts(self, users: list, sleep_time=10, between=10):
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    return self.console.log("Conta detectada como BOT")
            except Exception as error:
                self.console.log(
                    f"Erro ao fazer login no Instagram: Conta detectada como BOT"
                )
                raise error
        try:
        ...
        try:
            ...
            self.driver.quit()
            self.console.log(
                f"O perfil do usuario: [red]{error.user}[/] é privado, portanto é impossivel pegar os posts deste perfil!"
            )
            raise error
            ...
            self.driver.quit()
            self.console.log(
                f"Error ao pegar informação dos POSTs: {error.__class__.__name__}",
            )
            raise error

    def __check_is_private(self, user):
    ...
    def __scrapings_link_posts(self, n: int, max_failed_scroll=300) -> list:
        ...
        scrolling = True
        with self.console.status(
            f"[green]Realizando o scraping dos {n} link posts deste perfil!"
        ) as scraping:
            while scrolling:
